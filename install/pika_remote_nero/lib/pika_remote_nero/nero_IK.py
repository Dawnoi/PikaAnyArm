#!/usr/bin/env python3
"""
Nero Inverse Kinematics Node
接收目标末端位姿，使用数值优化求解关节角度并发布
"""
...
class RosOperator(Node):
    def __init__(self, args):
        super().__init__(f'nero_IK{args.index_name}')
        self.args = args
    ...
    def arm_joint_state_ctrl(self, joint_state):
        self.last_ctrl_arm_joint_state = joint_state[:7]  # Nero 有 7 个关节
    ...
    def arm_joint_state_ctrl_linear_interpolation(self, joint_state):
        ...
        joint_state_diff = max(abs(self.last_ctrl_arm_joint_state - joint_state[:7]))
        if joint_state_diff > 30.0 / 180 * 3.1415926:
            print("max_diff:", joint_state_diff / 3.1415926 * 180)
            step = int(joint_state_diff / (1.0 / 180 * 3.1415926))
            hz = 200
            rate = self.create_rate(hz)
            joint_state_list = np.linspace(self.last_ctrl_arm_joint_state, joint_state[:7], step + 1)
    ...
    def arm_joint_state_callback(self, msg):
        self.arm_joint_state = np.array(msg.position[:7])
    ...
    def publishing(self):
        ...
        while rclpy.ok():
            ...
            joint_position = []
            for i in range(7):
                joint_position.append(sol_q[i])

            joint_state_msg = JointState()
            joint_state_msg.header = Header()
            joint_state_msg.header.stamp = self.get_clock().now().to_msg()
            joint_state_msg.name = [f'joint{i+1}' for i in range(7)]
            ...
            # 计算 FK 验证
            xyzrpy = self.arm_ik.get_pose(sol_q[:7])
    ...
    def init_params(self):
        ...
        self.declare_parameter('gripper_xyzrpy', "[0.032, 0.0, -0.0235, -1.5708, 0.0, -1.5708]")
    ...
    def init_ros(self):
        self.arm_end_pose_subscriber = self.create_subscription(PoseStamped, f'/nero_IK{self.args.index_name}/ctrl_end_pose', self.arm_end_pose_callback, 1)
        ...
        self.arm_end_pose_orient_publisher = self.create_publisher(PoseStamped, f'/nero_IK{self.args.index_name}/urdf_end_pose_orient', 1)
        if not self.args.use_orient:
            self.arm_end_pose_publisher = self.create_publisher(PoseStamped, f'/nero_IK{self.args.index_name}/urdf_end_pose', 1)
            self.arm_receive_end_pose_publisher = self.create_publisher(PoseStamped, f'/nero_IK{self.args.index_name}/receive_end_pose_orient', 1)
...
def get_arguments():
    ...
    parser.add_argument('--gripper_xyzrpy', action='store', nargs='+', type=float, help='gripper_xyzrpy',
                        default=[0.032, 0.0, -0.0235, -1.5708, 0.0, -1.5708], required=False)
