...
class RosOperator(Node):
    def __init__(self, args):
        super().__init__(f'nero_FK{args.index_name}')
        self.args = args
    ...
    def calc(self):
        ...
        while rclpy.ok():
            if (self.args.lift and self.lift_msg is None) or self.arm_msg is None:
                rate.sleep()
                continue
            # 获取关节角度 (lift关节 + 7个Nero关节)
            q = (self.lift_msg.position if self.args.lift else []) + list(self.arm_msg.position[:7])
            
            xyzrpy = self.arm_fk.get_pose(q)
    ...
    def init_ros(self):
        ...
        self.arm_end_pose_publisher = self.create_publisher(PoseStamped, f'/nero_FK{self.args.index_name}/urdf_end_pose', 1)
        self.arm_end_pose_orient_publisher = self.create_publisher(PoseStamped, f'/nero_FK{self.args.index_name}/urdf_end_pose_orient', 1)
